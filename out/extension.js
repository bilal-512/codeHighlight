...
let overlayDecoration;
let topBorderDecoration;
let bottomBorderDecoration;
let borderDecoration;
let singleLineBorderDecoration;
let dimDecoration;
let disposableSelection;
let disposableMouseDown;
let disposableMouseMove;
let disposableMouseUp;
let isActivated = false;
let isSelecting = false;
let selectionStart;
let selectionEnd;
let currentFocusRange;
...
function activate(context) {
    ...
    context.subscriptions.push(toggleCommand, activateCommand, deactivateCommand, clearCommand, startSelectionCommand, editorChangeDisposable, documentChangeDisposable);
}
...
async function createFocusArea(editor, start, end) {
    ...
    try {
        ...
        const validStart = new vscode.Position(Math.max(0, Math.min(actualStart.line, maxLine)), 0);
        const validEnd = new vscode.Position(Math.max(0, Math.min(actualEnd.line, maxLine)), editor.document.lineAt(Math.min(actualEnd.line, maxLine)).text.length);
...
function updateDecorations(editor, focusRange) {
    ...
    try {
        ...
        const dimRanges = [];
        const focusRanges = [];
        const topBorderRanges = [];
        const bottomBorderRanges = [];
        const sideBorderRanges = [];
        const singleLineBorderRanges = [];
        // Validate focus range bounds
        const maxLine = document.lineCount - 1;
        const validFocusRange = new vscode.Range(new vscode.Position(Math.max(0, Math.min(focusRange.start.line, maxLine)), 0), new vscode.Position(Math.max(0, Math.min(focusRange.end.line, maxLine)), Math.min(focusRange.end.character, document.lineAt(Math.min(focusRange.end.line, maxLine)).text.length)));
        ...
        if (isSingleLine) {
            ...
            if (validFocusRange.start.line >= 0 && validFocusRange.start.line < document.lineCount) {
                singleLineBorderRanges.push(new vscode.Range(validFocusRange.start.line, 0, validFocusRange.start.line, maxLineWidth));
            }
        }
        else {
            // Apply top border to the first line with uniform width
            if (validFocusRange.start.line >= 0 && validFocusRange.start.line < document.lineCount) {
                topBorderRanges.push(new vscode.Range(validFocusRange.start.line, 0, validFocusRange.start.line, maxLineWidth));
            }
            ...
            if (validFocusRange.end.line >= 0 && validFocusRange.end.line < document.lineCount) {
                bottomBorderRanges.push(new vscode.Range(validFocusRange.end.line, 0, validFocusRange.end.line, maxLineWidth));
            }
...
function clearFocusArea(showMessage = true) {
    const editor = vscode.window.activeTextEditor;
    if (!editor)
        return;
